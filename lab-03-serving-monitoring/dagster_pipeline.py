...
from training_utils.wrapper import BiomassPyFuncModel
...
#Bild → eine Zahl (Durchschnitts-Helligkeit) → zum Vergleichen von alten vs. neuen Bildern.
def _mean_gray_pixel(image_path: Path) -> float:
...
#Referenz = alte (Trainings-)Bilder → zum Vergleichen mit neuen Bildern.
#Datenquelle=Training (alt)
@asset
def reference_mean_pixel(context: AssetExecutionContext):
    img_dir = Path(IMAGES_DIR)
    paths = sorted([p for p in img_dir.iterdir() if p.suffix.lower() in [".jpg", ".jpeg", ".png"]])

    if not paths:
        raise RuntimeError(f"No images found in training folder: {img_dir}")

    # optional: limit (damit es schneller ist)
    paths = paths[:200]

    values = []
    for p in paths:
        try:
            values.append(_mean_gray_pixel(p))
        except Exception as e:
            context.log.warning(f"Skip broken image {p.name}: {e}")

    df = pd.DataFrame({"mean_pixel": values})
    context.log.info(f"Reference images used: {len(df)}")

    context.add_output_metadata({
        "n_reference_images": len(df),
        "mean_of_mean_pixel": MetadataValue.float(float(df["mean_pixel"].mean()))
    })
    return df


#Produktionsdaten in Zahlen umwandeln, um sie später mit den Trainingsdaten zu vergleichen.
#Datenquelle=Produktion (neu)
@asset
def production_mean_pixel(context: AssetExecutionContext):
    prod_dir = Path("production_data") / "images"
    prod_dir.mkdir(parents=True, exist_ok=True)

    paths = sorted([p for p in prod_dir.iterdir() if p.suffix.lower() in [".jpg", ".jpeg", ".png"]])

    if not paths:
        # Kein Crash: einfach sagen „keine Daten“
        context.log.warning("No production images found yet in production_data/images/. Upload images in Gradio first.")
        df = pd.DataFrame({"mean_pixel": []})
        context.add_output_metadata({"n_production_images": 0})
        return df
    ...
    for p in paths:
        try:
            ...
            context.log.warning(f"Skip broken production image {p.name}: {e}")

    df = pd.DataFrame({"mean_pixel": values})
    context.log.info(f"Production images used: {len(df)}")

    context.add_output_metadata({
        "n_production_images": len(df),
        "mean_of_mean_pixel": MetadataValue.float(float(df["mean_pixel"].mean()))
    })
    return df


#Dieses Asset vergleicht die alten Daten mit den neuen Daten und sagt dir
...
def drift_report(context: AssetExecutionContext, reference_mean_pixel: pd.DataFrame, production_mean_pixel: pd.DataFrame):
    ...
    reference_sample = reference_mean_pixel[["mean_pixel"]].copy()
    current_sample   = production_mean_pixel[["mean_pixel"]].copy()

    # Evidently Report
    report = Report(metrics=[DataDriftPreset()])
    snapshot = report.run(reference_data=reference_sample, current_data=current_sample)
    result_dict = snapshot.dict()

    # Drift-Infos rausziehen
    drifted_columns_metric = next(
        m for m in result_dict["metrics"] if "DriftedColumnsCount" in m["metric_name"]
    )
    value_drift_metric = next(
        m for m in result_dict["metrics"] if "ValueDrift" in m["metric_name"]
    )

    drift_detected = drifted_columns_metric["value"]["count"] > 0
    drift_score = value_drift_metric["value"]  # bei 1 Feature ist das der Score fürs mean_pixel

    # HTML speichern
    output_path = RESULTS_DIR / "drift_report.html"
    snapshot.save_html(str(output_path))

    # Logs + Metadaten für Dagster UI
    context.log.info(f"Drift detected: {drift_detected}")
    context.log.info(f"Drift score: {drift_score}")
    context.log.info(f"Saved drift report: {output_path}")

    context.add_output_metadata({
        "drift_detected": MetadataValue.bool(bool(drift_detected)),
        "drift_score": MetadataValue.float(float(drift_score) if drift_score is not None else float("nan")),
...
def trained_model(context: AssetExecutionContext, preprocessed_data):
    ...
    artifact_path="model",
    python_model=BiomassPyFuncModel(),
    artifacts={"weights": str(model_path)},   # model.pt
    code_paths=["training_utils"],            # wichtig: wrapper mit speichern
    )
...
defs = Definitions(
    ...
    assets=[raw_dataset, eda_plots, preprocessed_data, trained_model, model_evaluation,reference_mean_pixel,production_mean_pixel,drift_report,],
    resources={
